...
def crear(root):# esta es la ventana de recepción de datos
    ...
    def guardar_ruta_en_json(ruta_imagen):
...
##            self.colorwin = colorwin            
    ...
    def checkbuttonds(frame,memory,specs,colorwin,opcion,display = "None"):
        texto = ""
        def the_checkbutton_funk():
            seleccion = []
...
##    picto_prod_display = tk.Label(, text="Str pictogramas max 3", font=('Arial', 9),bg=colores[1],fg="#ffffff")
##    picto_prod_display.grid(row=3, column=0)
##    # ENTRY                    
##    picopciones1 = {}
...
##    opciones.grid(row=0, column=1)
    ...
    for i,j in enumerate(specs):# PARA CADA ESPECIFICACION DE "REACTIVOS"...
        ...
        if j == 'pictogramas':
            picopciones2 = {}
            d8 = tk.LabelFrame(option_frames[lista_opciones[0]],text="Pictogramas (max 3)",bg=colores[1])# Checkbuttons for the pictogramas
            r_picto, chkbtns2 = checkbuttonds(option_frames[lista_opciones[0]],picopciones2,pspecs,colores[1])
            d8.grid(row=row+i,column=1)
        else:
    ...
    ttk.Button(option_frames[lista_opciones[2]], text="Seleccionar Imagen", command=seleccionar_imagen).grid(row=row+len(specs), column=1)
    etiqueta_imagen = tk.Label(option_frames[lista_opciones[2]])
    etiqueta_imagen.grid(row=row+len(specs)+1, column=0)
